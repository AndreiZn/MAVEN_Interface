function [message, error] = Mass_Spectrum_normalized (ax, filefolderpath, date, start_time, stop_time, specific_args)
    ...
    if ~isempty(filename)
        ...
        if log==1
            semilogy (squeeze(mass_arr(16, swp_ind(timenum)+1, :)), flux_sum, 'linewidth', 0.5);
        else
            plot (squeeze(mass_arr(16, swp_ind(timenum)+1, :)), flux_sum, 'linewidth', 0.5);
        end
...
function [file, msg, err] = get_file(date, filefolderpath)
    ...
    filetype = 'c6_32e64m';
    chosenfunc = 'Mass_Spectrum_normalized';
    extensions = {{'', '.cdf',}};
