function Speed_Time (ax, start_time, stop_time, filename, specific_args) %d1 file is being used
    ...
    axes(ax);
    v_sqrt = sqrt(v_mso(:, 1).^2+v_mso(:, 2).^2+v_mso(:, 3).^2)/1e3;
    temp_v = find (v_sqrt(:) > 1); 
    
    if (log==1)
        semilogy(epoch(temp_v), v_mso(temp_v, 1)/1e3, 'color', 'red', 'linewidth', 2)
        hold on
        semilogy(epoch(temp_v), v_mso(temp_v, 2)/1e3, 'color', 'green', 'linewidth', 2)
        semilogy(epoch(temp_v), v_mso(temp_v, 3)/1e3, 'color', 'blue', 'linewidth', 2)
        semilogy(epoch(temp_v), v_sqrt, 'color', 'black', 'linewidth', 2)
    else
        plot(epoch(temp_v), v_mso(temp_v, 1)/1e3, 'color', 'red', 'linewidth', 2)
        hold on
        plot(epoch(temp_v), v_mso(temp_v, 2)/1e3, 'color', 'green', 'linewidth', 2)
        plot(epoch(temp_v), v_mso(temp_v, 3)/1e3, 'color', 'blue', 'linewidth', 2)
        plot(epoch(temp_v), v_sqrt, 'color', 'black', 'linewidth', 2)
    end
    
    legendhndl = legend('V_x MSO', 'V_y MSO', 'V_z MSO', 'V');
    datetick('x','HH:MM:SS');
    grid on
    ylabel('V, km/s')
    set (ax, 'fontsize', 8);
