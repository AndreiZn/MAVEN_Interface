function [message, error] = nO_divided_by_np (ax, filefolderpath, date, start_time, stop_time, specific_args)
    
    [filename, message, error] = get_file(date, filefolderpath);
    if ~isempty(filename)

        log = specific_args{1, 1};
        ...
        ind = 1; % ind is from 1 to 3 and refers to the mass type
        c = zeros(length(epoch), 3);
        ...
        for mass=[1, 16, 32]
            ...
            c(1:length(epoch), ind) = concentration(:,1);
            ind = ind + 1;
