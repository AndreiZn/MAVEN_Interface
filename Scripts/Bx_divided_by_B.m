function [message, error] = Bx_divided_by_B (ax, filefolderpath, date, start_time, stop_time, specific_args)
    ...
    if strcmp(file_type, 'mag')
        ...
        if ~isempty(mf_filename)
            ...
            plot(mf_data2(:,7), Bx./B, 'linewidth', 0.5)
    ...
    elseif strcmp(file_type, 'sql')
        ...
        sqlGetData = strcat(['SELECT OB_B_X, OB_B_Y, OB_B_Z, DDAY FROM maven.mag where TIME_YEAR=', time_year,' and DDAY>=', ddayfrom,' and DDAY<=', ddayto]);
        curs = exec(conn,sqlGetData); %curs = exec(conn,'SELECT * from maven.key_parameters');
        curs = fetch(curs);
        table = curs.Data;
        close (curs);
        
        Bx = [table{:,1}]; By = [table{:,2}]; Bz = [table{:,3}];
        B = sqrt(Bx.^2 + By.^2 + Bz.^2);
        dday = [table{:,4}];
        
        axes(ax); 

        plot(dday, Bx./B, 'linewidth', 0.5)
