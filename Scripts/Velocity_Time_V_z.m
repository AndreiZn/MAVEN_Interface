function Velocity_Time_V_z (ax, start_time, stop_time, filename, specific_args) %d1 file is being used
    ...
    if (log==1)
        semilogy(epoch, v_mso(:, 3)/1e3, 'color', 'red', 'linewidth', 2)
    else
        plot(epoch, v_mso(:, 3)/1e3, 'color', 'red', 'linewidth', 2)
    end
