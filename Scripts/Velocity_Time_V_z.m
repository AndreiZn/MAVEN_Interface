function [message, error] = Velocity_Time_V_z (ax, filefolderpath, date, start_time, stop_time, specific_args)
    ...
    if strcmp(file_type, 'd1')
        ...
        if ~isempty(filename)
            ...
            if (log==1)
                semilogy(epoch, v_mso(:, 3)/1e3, 'linewidth', 0.5)
            else
                plot(epoch, v_mso(:, 3)/1e3, 'linewidth', 0.5)
            end
    ...
    elseif strcmp(file_type, 'sql')
        ...
        if mass == 1
            col_name = {'ts_v_mso_z_H'};
            ylab = 'V_H^z, km/s';
        elseif mass == 16
            col_name = {'ts_v_mso_z_O'};
            ylab = 'V_O^z, km/s';
        elseif mass == 32
            col_name = {'ts_v_mso_z_O2'};
            ylab = 'V_{O^2}^z, km/s';    
        end
