function [message, error] = Velocity_Time_V_x (ax, filefolderpath, date, start_time, stop_time, specific_args)
    ...
    if strcmp(file_type, 'd1')
        ...
        if ~isempty(filename)
            ...
            if (log==1)
                semilogy(epoch, v_mso(:, 1)/1e3, 'linewidth', 0.5)
            else
                plot(epoch, v_mso(:, 1)/1e3, 'linewidth', 0.5)
            end
    ...
    elseif strcmp(file_type, 'sql')
        ...
        if mass == 1
            col_name = {'ts_v_mso_x_H'};
            ylab = 'V_H^x, km/s';
        elseif mass == 16
            col_name = {'ts_v_mso_x_O'};
            ylab = 'V_O^x, km/s';
        elseif mass == 32
            col_name = {'ts_v_mso_x_O2'};
            ylab = 'V_{O^2}^x, km/s';    
        end
