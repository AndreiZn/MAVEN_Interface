function [message, error] = Velocity_Time_V_y (ax, filefolderpath, date, start_time, stop_time, specific_args)
    ...
    if strcmp(file_type, 'd1')
        ...
        if ~isempty(filename)
            ...
            if (log==1)
                semilogy(epoch, v_mso(:, 2)/1e3, 'linewidth', 0.5)
            else
                plot(epoch, v_mso(:, 2)/1e3, 'linewidth', 0.5)
            end
    ...
    elseif strcmp(file_type, 'sql')
        ...
        if mass == 1
            col_name = {'ts_v_mso_y_H'};
            ylab = 'V_H^y, km/s';
        elseif mass == 16
            col_name = {'ts_v_mso_y_O'};
            ylab = 'V_O^y, km/s';
        elseif mass == 32
            col_name = {'ts_v_mso_y_O2'};
            ylab = 'V_{O^2}^y, km/s';    
        end
